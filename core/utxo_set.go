...
func (u UTXOSet) FindSpendableOutputs(pubkeyHash []byte, amount float64) (float64, map[string][]int) {
	...
	var accumulated float64=0.0//sender发出的转出的全部币数
...
func (u *UXTOSet) DeleteByPrefix(prefix []byte) {
	...
	collectSize := 100000
	u.Blockchain.Database.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(utxoBucket))
		c := b.Cursor()
		...
		keysCollected := 0
		for c.Seek(prefix); c.ValidForPrefix(prefix); c.Next() {
			key := c.Item().KeyCopy(nil)
