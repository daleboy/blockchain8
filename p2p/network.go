...
import (
	...
	"sd-chain/blockchain8/memopool"
	appUtils "sd-chain/blockchain8/util/utils"
)
...
func (net *Network) HandleBlocks(content *ChannelContent) {
	...
	if block.IsGenesis() {
		...
		lastBlock, err := net.Blockchain.GetBlock(net.Blockchain.Tip)
		if err != nil {
			log.Info(err)
		}
	...
	if len(blocksInTransit) > 0 {
		...
		UTXO := blockchain.UTXOSet{net.Blockchain}
		UTXO.Compute()
	}
}
...
func (net *Network) MineTx(memopoolTxs map[string]blockchain.Transaction) {
	...
	UTXOs := blockchain.UTXOSet{chain}
	UTXOs.Compute()
